...
class Unet(nn.Module):
    def __init__(
        ...
        init_filters = default(init_filters, self.filters)

        dims = [init_filters, *map(lambda m: self.filters * m, filter_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))

        block_klass = partial(ResnetBlock, groups=resnet_block_groups)

        # time embeddings
        time_dim = self.filters * 4
        ...
        # class embeddings
        self.class_embeddings = nn.Embedding(num_classes, class_embed_dim)

        # initial convolusion
        self.init_conv = nn.Conv2d(input_channels, init_filters, 1, padding=0)
...
class MyDataset(Dataset):
    def __init__(self, drive_path):
        self.pixel_values = np.expand_dims(np.load(os.path.join(drive_path, 'save_data.npy')), axis = 1)
        self.labels = np.load(os.path.join(drive_path, 'save_labels.npy'))
...
save_path = 'C:/Users/kunal/Documents/BoekeLabResearch/diffusion/binding_sites_weights.pth'
model.load_state_dict(torch.load(save_path))

# sample 64 images
batch_size = 64
classr = 9
samples = sample(model=model, image_size=image_size, \
                 class_vector = torch.tensor(np.array([classr] * batch_size), device=device), \
                 batch_size=batch_size, channels=channels)

#random showing
plt.imshow(samples[-1][9].reshape(image_size[0], image_size[1], channels), cmap="gray")

#avg showing
avg = np.zeros((image_size[0], image_size[1]))
for i in range(len(samples[0])):
  a = samples[-1][i].reshape(image_size[0], image_size[1])
  b = len(samples[0])
  avg += (a / b)
