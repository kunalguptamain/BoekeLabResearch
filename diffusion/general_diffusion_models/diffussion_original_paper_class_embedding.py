...
class ClassConditioning(nn.Module):
    def __init__(self, in_dims, res, num_channels=1):
        super().__init__()
        self.block = nn.Sequential(
            nn.Linear(in_dims, res * res * num_channels),
            nn.SiLU(),
            nn.Unflatten(1, (num_channels, res, res))
        )
...
num_classes = transformed_dataset["train"].features["label"].num_classes
...
def p_sample(model, x, t, t_index):
    ...
    model_mean = sqrt_recip_alphas_t * (
        x - betas_t * model(x, t, torch.tensor(np.array([0]*64))) / sqrt_one_minus_alphas_cumprod_t
    )
